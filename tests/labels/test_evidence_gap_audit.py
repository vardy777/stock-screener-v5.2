...
def test_real_gap_audit_covers_frozen_inventory_without_vague_root_causes():
    ...
    assert all(entry.blocker_class in BLOCKER_CLASSES for entry in audit.entries)
    assert all(entry.blocker_reason != "EVIDENCE_UNAVAILABLE" for entry in audit.entries)
    assert audit.verify()


def test_gap_audit_distinguishes_missing_old_bars_from_unimplemented_assembly():
    audit = audit_phase2a_evidence_gaps(ROOT)
    ...
    assert by_slot[9].blocker_class == "ASSEMBLER_LOOKUP_DEFECT"
    ...
    assert by_slot[14].blocker_class == "ASSEMBLER_LOOKUP_DEFECT"
...
def test_gap_matrix_renders_each_frozen_slot_and_exact_blocker_counts():
    ...
    assert report.count("\n| 01 |") == 1
    assert report.count("\n| 22 |") == 1
    assert "ASSEMBLER_LOOKUP_DEFECT COUNT = 22" in report
